  ...
  try {
    ...
    const [messages, totalCount] = await Promise.all([
      ChatMessage.find({ conversationId })
        .populate('senderId', userSelect)
        .populate('receiverId', userSelect)
        .sort({ createdAt: -1 }) // Get newest first, then reverse if needed
        .limit(limit)
        .skip(skip)
        .lean(),
  ...
  try {
    ...
    const [messages, totalCount] = await Promise.all([
      ChatMessage.find({ teamleaderid })
        ...
        .skip(skip)
        .lean(),
      ChatMessage.countDocuments({ teamleaderid })
    ]);
  ...
  try {
    ...
    const [messages, totalCount] = await Promise.all([
      ChatMessage.find({ managerid })
        ...
        .skip(skip)
        .lean(),
      ChatMessage.countDocuments({ managerid })
    ]);
